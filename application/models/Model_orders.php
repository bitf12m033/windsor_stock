...
class Model_orders extends CI_Model
{
	...
	public function countTotalPaidOrdersByStore($store_id)
	{
		$this->db->select('orders.*');
		$this->db->from('orders');
		$this->db->join('users', 'users.id = orders.user_id');
		$this->db->where('users.store_id', $store_id);
		$this->db->where('orders.paid_status', 1); // Assuming 1 indicates a paid order
		return $this->db->count_all_results();
	}

	public function getOrdersDataByStore($store_id)
	{
		$this->db->select('orders.*');
		$this->db->from('orders');
		$this->db->join('users', 'users.id = orders.user_id');
