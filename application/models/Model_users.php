...
class Model_users extends CI_Model
{
	...
	public function getUserData($userId = null) 
	{
		...
		$sql = "SELECT users.* , stores.name as store_name FROM `users` LEFT JOIN stores ON stores.id = users.store_id WHERE users.id != ?";
	...
	public function countTotalUsersByStore($store_id)
	{
		$this->db->where('store_id', $store_id);
		$this->db->from('users');
		return $this->db->count_all_results();
	}
	public function getUserDataByStore($store_id)
	{
		if($store_id) {
			$this->db->select('users.*, stores.name as store_name');
			$this->db->from('users');
			$this->db->join('stores', 'stores.id = users.store_id');
			$this->db->where('users.store_id', $store_id);
			$query = $this->db->get();
			return $query->result_array();
		}
		return array();
