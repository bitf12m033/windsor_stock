    ...
    */
	public function fetchProductData()
    {
        ...
        if ($this->session->userdata('is_admin')) {
            $data = $this->model_products->getProductData(null, $search);
        } else {
            // Fetch products only for the user's store
            $store_id = $this->session->userdata('store_id');
            $data = $this->model_products->getProductDataByStore($store_id, $search);
        }
        ...
        foreach ($data as $key => $value) {
            ...
            if(in_array('viewMarkSold', $this->permission) && $value['availability'] == 1) {
